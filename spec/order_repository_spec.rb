...
RSpec.describe OrderRepository do
    ...
    it 'Get all orders' do
        ...
        expect(orders[1].customer_name).to eq "Robby Dobby"
        expect(orders[0].order_date).to eq '6 Jan 2023'
    ...
    it 'Adds a new order' do
        repo = OrderRepository.new
        order = Order.new
        order.customer_name = 'Yo Yo Ma'
        order.order_date = '7 Jan 2023'
        order.item_id = 1
        repo.create(order)
        all_orders = repo.all
        expect(all_orders.length).to eq 3
        expect(all_orders.last.id).to eq 3
        expect(all_orders.last.customer_name).to eq 'Yo Yo Ma'
